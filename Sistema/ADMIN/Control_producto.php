...
ob_start(); 
  header("refresh: 3; url = revisar_stock.php"); 
...
ob_end_flush();
    ...
    if(!$connection){ //Si la conexión falla...
        ...
        if(isset($_POST['ID_I'])){ 
            ...
            $query = OCIParse($connection, "begin INGRESA_PRODUCTO(:ID_I, :NOMBRE_I, :PRECIO_I, :STOCK_I,:CATEGORIA_I, :retorno, :er); end;");
            ...
            oci_bind_by_name($query, ':PRECIO_I',$PRECIO_I);
            oci_bind_by_name($query, ':STOCK_I',$STOCK_I);
            oci_bind_by_name($query, ':CATEGORIA_I',$CATEGORIA_I);
        ...
        if(isset($_POST['ID_B'])){ 
            ...
            //Con OCIParse se hace la conversión de php a sql
            $query = OCIParse($connection, "begin DELETE_PRODUCTO(:ID_I, :retorno, :er); end;");
            //Los oci_bind se encargar de vincular las variables del php con las del procedimiento
            oci_bind_by_name($query, ':ID_I', $ID_I);
        ...
        if(isset($_POST['ID_U'])){ 
            ...
            //Con OCIParse se hace la conversión de php a sql
            $query = OCIParse($connection, "begin ACTUALIZAR_PRODUCTO(:ID_U, :NOMBRE_U, :PRECIO_U, :STOCK_U,:CATEGORIA_U, :retorno, :er); end;");
            //Los oci_bind se encargar de vincular las variables del php con las del procedimiento
            oci_bind_by_name($query, ':ID_U', $ID_U);
            oci_bind_by_name($query, ':NOMBRE_U',$NOMBRE_U);
            oci_bind_by_name($query, ':PRECIO_U',$PRECIO_U);
            oci_bind_by_name($query, ':STOCK_U',$STOCK_U);
            oci_bind_by_name($query, ':CATEGORIA_U',$CATEGORIA_U);
